       ...
       {#each datapoints_vis1 as datapoint}
        {#if datapoint.Coord_X > 1 && datapoint.year_month == mapSliderToDateString(slider_value)
            ...
            <g class = 'custom_class'>
                <path id="conePath" class = 'circle' d=
                "M  {datapoint.Coord_X * rescaling_factor + margins.left} {datapoint.Coord_Y * rescaling_factor + margins.top - (datapoint.revenue/max_revenue) * (500 * rescaling_factor)}
                L {datapoint.Coord_X * rescaling_factor + margins.left + cone_radius / 2} {datapoint.Coord_Y * rescaling_factor + margins.top}
                L {datapoint.Coord_X * rescaling_factor + margins.left - cone_radius / 2} {datapoint.Coord_Y * rescaling_factor + margins.top}
                Z"
                fill-opacity = "0.8"
                fill = #4D4D4D 
            ...
            <path id="conePath" class = 'circle' d=
                "M  {(datapoint.Coord_X * rescaling_factor) + map_width + margins.left + between_plot_margin} {datapoint.Coord_Y * rescaling_factor + margins.top - ((datapoint.revenue/datapoint.number_clients)/max_rev_per_client) * (500 * rescaling_factor)}
                L {(datapoint.Coord_X * rescaling_factor) + map_width + margins.left + between_plot_margin + cone_radius / 2} {datapoint.Coord_Y * rescaling_factor + margins.top}
                L {(datapoint.Coord_X * rescaling_factor) + map_width + margins.left + between_plot_margin - cone_radius / 2} {datapoint.Coord_Y * rescaling_factor + margins.top}
                Z"
                fill-opacity = "0.8"
                fill = #4D4D4D 
